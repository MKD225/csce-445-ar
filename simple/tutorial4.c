...
void init(void)
{
    ...
    Load3DS (&object,"models/sofa.3DS");
	//Load3DS (&object,"spaceship.3ds");

    object.id_texture=LoadBitmap("models/blueleather.bmp"); // The Function LoadBitmap() return the current texture ID
...
void display(void)
{
	...
	glTranslatef(deltaX,0.0,deltaZ); // We move the object forward (the model matrix is multiplied by the translation matrix)
